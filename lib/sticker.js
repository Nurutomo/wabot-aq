...
async function addExif(webpSticker, packname, author, categories = [''], extra = {}) {
  ...
  const img = new webp.Image();
  const stickerPackId = crypto.randomBytes(32).toString('hex');
  const packid = 'com.client.tech'//DONT DELETE THIS
  const plink = 'https://play.google.com/store/apps/details?id=com.termux'//DONT DELETE THIS
 const alink = 'https://itunes.apple.com/app/sticker-maker-studio/id1443326857'//DONT DELETE THIS
  const json = { 'sticker-pack-id': packid, 'sticker-pack-name': packname, 'sticker-pack-publisher': author, 'android-app-store-link': plink, 'Ios-app-store-link': alink, 'emojis': categories, ...extra };
  let exifAttr = Buffer.from([0x49, 0x49, 0x2A, 0x00, 0x08, 0x00, 0x00, 0x00, 0x01, 0x00, 0x41, 0x57, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00]);
  let jsonBuffer = Buffer.from(JSON.stringify(json), 'utf8');
  let exif = Buffer.concat([exifAttr, jsonBuffer]);
  exif.writeUIntLE(jsonBuffer.length, 14, 4);
