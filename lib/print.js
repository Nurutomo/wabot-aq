...
`.trim(),
  ...
  let m_messageStubParameters = m.messageStubParameters
  let messageStubParameters = ''
  if (m_messageStubParameters) for (let jid of m_messageStubParameters) {
    let name = await conn.getName(jid)
    messageStubParameters += chalk.gray(PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international') + (name ? ' ~' + name : '') + m_messageStubParameters.indexOf(jid) > 0 && m_messageStubParameters.indexOf(jid) !== (m_messageStubParameters.length - 1) ? ', ' : '' )
  }
  console.log(messageStubParameters)
  
  /*
  console.log(m.messageStubParameters.map(async jid => {
    let name = await conn.getName(jid)
