...
module.exports = async function (m, conn = {user: {}}) {
  ...
  console.log(
    `${chalk.redBright('%s')} ${chalk.inverse(chalk.yellow('%s'))} ${chalk.inverse(chalk.green('%s'))}\n${chalk.green('%s')} ${chalk.yellow('%s%s')} ${chalk.blueBright('to')} ${chalk.green('%s')} ${chalk.inverse(chalk.yellow('%s'))} ${chalk.magenta('%s |%d %sB]')}`,
    me + ' ~' + conn.user.name,
