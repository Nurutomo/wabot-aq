    ...
    */
    ...
    getName(jid, withoutContact = false) {
      ...
      if (jid.endsWith('@g.us')) {
        ...
        v = chat && chat.metadata ? chat.metadata : chat
        if (!(v.name || v.subject) || true) this.fetchGroupMetadataFromWA(jid).then(gc => {
          if (gc.subject) v = gc
          else this.groupMetadata(jid).then(group => {
            if (group.subject) v = group
            else {
              let chats = this.chats.get(jid)
              let metadata = chats && chats.metadata ? chats.metadata : undefined
              if (!metadata) {
                if (chats.read_only ? chats.read_only : false) this.groupMetadataMinimal(jid).then(meta => metadata = meta)
                else this.fetchGroupMetadataFromWA(jid).then(meta => metadata = meta)
                if (chats) chats.metadata = metadata
              }
              v = metadata 
            }
          })
      ...
      if (jid === this.user.jid) v = this.user || {}
