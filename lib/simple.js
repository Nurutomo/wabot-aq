...
exports.WAConnection = (_WAConnection) => {
	class WAConnection extends _WAConnection {
  	...
  	async sendFile(jid, path, filename = '', caption = '', quoted, ptt = false, options) {
	  	const file = Buffer.isBuffer(path) ? path : path.startsWith('data:') ? Buffer.from(path) : path.startsWith('http') ? await (await fetch(path)).buffer() : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
  		...
  		else mtype = MessageType.document
	  ...
	  fakeReply(jid, text = '', fakeJid = this.user.jid, fakeText = '', fakeGroupJid = '') {
  		return this.reply(jid, text, { key: { fromMe: fakeJid == this.user.jid, participant: fakeJid, remoteJid: fakeGroupJid }, message: { conversation: fakeText }})
  	}
	  ...
	  getName(jid)  {
  		let v = jid === this.user.jid ? this.user : this.contacs[id] || { notify: id.replace(/@.+/, '') }
  		return v.name || v.vname || v.notify
  	}

	  async downloadM(m) {
    	if (!m.message) return Buffer.alloc(0)
	  	if (m.message[Object.keys(m.message)[0]].url) await conn.updateMediaMessage(m)
  		return await conn.downloadMediaMessage(m)
  	}
	}
