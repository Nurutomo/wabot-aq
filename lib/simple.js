...
`.trim()
    ...
    */
     ...
     * @param {Object} quoted
     * @param {Object} options
     * @example sendButtonsText this.sendButtons(m.chat, 'content', 'footer', null, [['Button text', 'Buttonid'], ['Button text 1', 'button id']], m)
     * @example sendButtonsImage this.sendButtons(m.chat, 'content', 'footer', buffer, [['Button text', 'Buttonid'], ['Button text 1', 'button id']], m)
     * @example sendButtonsLocation this.sendButtons(m.chat, 'content', 'footer', locationBuffer, [['Button text', 'Buttonid'], ['Button text 1', 'button id']], m, { loc: true })
     */
    async sendButtons(jid, content, footer, buffer = null, button = [], quoted, options = {}) {
      ...
      return await this.sendMessage(jid, buttonMessage, MessageType.buttonsMessage, { quoted, ...options })
    }
    ...
    getName(jid, withoutContact = false) {
      ...
      if (jid.endsWith('@g.us')) {
        let groupMeta = async () => {
          let gc = await this.chats.get(jid) || {}
          v = gc && gc.metadata ? gc.metadata : (gc || {})
          if (!(v.name || v.subject)) {
            let group = await this.fetchGroupMetadataFromWA(jid).catch(_ => '{}')
            let chat = await this.groupMetadata(jid).catch(_ => '{}')
            v = group && group.subject ? group : chat && chat.subject ? chat : {}
            if (!(v.name || v.subject) && gc) {
              if (gc.read_only) v = await this.groupMetadataMinimal(jid).catch(_ => '{}')
              else v = await this.fetchGroupMetadataFromWA(jid).catch(_ => '{}')
              if (!(v.name || v.subject)) v = this.contactAddOrGet(jid) || {}
              return v
            } else return v
          } else return v
        }
        return groupMeta().then(res => {
          return (res.name || res.subject)
        }).catch(e => {
          console.error(e)
          return jid
        })
        ...
        } else if (jid === this.user.jid) v = this.user || {}
        else {
          v = this.contactAddOrGet(jid) || {}
          if (!(v.name || v.subject || v.vname || v.short || v.notify)) v = this.contacts[jid] || {}
        }
