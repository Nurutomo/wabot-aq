...
`.trim()
    ...
    async getFile(path) {
      ...
      if (options.asSticker) mtype = MessageType.sticker
        ...
        else if (/webp/.test(type.mime) && file.length <= 1 << 20) mtype = MessageType.sticker
    ...
    async fakeReply2(jid, message, type, sender, message2, type2, options, options2) {
      const content = await this.prepareMessageContent(message2, type2, options2)
      const quoted = this.prepareMessageFromContent(sender, content, options2)
      return this.sendMessage(jid, messags, type, { quoted, ...options })
    }
